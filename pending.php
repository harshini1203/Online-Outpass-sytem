...
$query = "SELECT a.out_no, b.name,a.go_date,a.return_date,a.reason,a.status FROM outpass a, student b where a.email=b.email and status='Waiting'";
$i=0;
$result = $mysqli->query($query);
if(!$result->fetch_assoc()){
  echo'<table border="1" cellspacing="40" cellpadding="20" style="left:420px;position:relative;color:white;"><tr>
  <td>
  ...
  echo '<button class="btn" onclick="Myfunc3()"><center><svg xmlns="http://www.w3.org/2000/svg"  width="20" height="16"  class="bi bi-patch-check-fill" viewBox="0 0 16 16" style="padding-right:10px;fill:white;">
    ...
    while ($row = $result->fetch_assoc()) {
      $i=$i+1;
        ...
        echo '<tr id='.$i.'>
                  ...
                  <td>'.$field4name.'</td>
                  <td>'.$field5name.'</td>
                  <td> <button class="tick" onclick="Myfunc2('.$i.')"><svg xmlns="http://www.w3.org/2000/svg"  width="30" height="46"  class="bi bi-patch-check-fill" viewBox="0 0 16 16">
  <path d="M10.067.87a2.89 2.89 0 0 0-4.134 0l-.622.638-.89-.011a2.89 2.89 0 0 0-2.924 2.924l.01.89-.636.622a2.89 2.89 0 0 0 0 4.134l.637.622-.011.89a2.89 2.89 0 0 0 2.924 2.924l.89-.01.622.636a2.89 2.89 0 0 0 4.134 0l.622-.637.89.011a2.89 2.89 0 0 0 2.924-2.924l-.01-.89.636-.622a2.89 2.89 0 0 0 0-4.134l-.637-.622.011-.89a2.89 2.89 0 0 0-2.924-2.924l-.89.01-.622-.636zm.287 5.984-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7 8.793l2.646-2.647a.5.5 0 0 1 .708.708z"/>
</svg></button>
                  <button class="wrong" onclick="Myfunc('.$i.')"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="46"  class="bi bi-x-circle-fill" viewBox="0 0 16 16">
...
<script>
  ...
  const p=u.children[0].innerHTML;
  $.ajax({
         type: 'POST',
         url: 'simple.php',
         data: {new_value:p}
      });
  alert("Outpass No."+ p +" is denied.");
}
function Myfunc2(i){
const u=document.getElementById(i);
const p=u.children[0].innerHTML;
$.ajax({
  ...
  alert("Outpass No."+ p +" is approved.");
}
function Myfunc3(){
...
$.ajax({
  ...
  alert("All outpasses are approved.");
}
function Myfunc4(i){
  const u=document.getElementById(i);
  const p=u.children[0].innerHTML;
$.ajax({
       type: 'POST',
       url: 'simple4.php?v=<?php echo time(); ?>',
       data: {new_value:p},
...
function Myfunc5(){
  window.location.href='simple4.php?v=<?php echo time(); ?>';
}
